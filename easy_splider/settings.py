...
from weibo_models import RequestInfo
...
PRIORITYDEFINE={
    'weibo_http_get_navigation_page_list':3,
    'weibo_http_get_navigation_page_url':2,
    'weibo_http_get_navigation':1
}
...
def init_log(filename=None):
    ...
    dirname, file = os.path.split(os.path.abspath(sys.argv[0]))
    ...
    fh = logging.handlers.TimedRotatingFileHandler(filename, when='D',interval=1, backupCount=3, encoding='utf8')
    fh.setLevel(logging.DEBUG)

    ch = logging.StreamHandler(sys.stdout)
    ch.setLevel(logging.WARNING)

    formater = logging.Formatter('%(asctime)s--%(filename)s--%(lineno)s--%(levelname)s--%(thread)d--%(funcName)s %(message)s')
